...
def load_configs_from_directory(
    ...
    # from the unzipped directory directly. Use safe_load to prevent unsafe
    # object construction during YAML deserialization.
    metadata = yaml.safe_load(contents.get(METADATA_FILE_NAME, "{}")) or {}
    if "type" in metadata:
        del metadata["type"]
    contents[METADATA_FILE_NAME] = yaml.safe_dump(metadata)
